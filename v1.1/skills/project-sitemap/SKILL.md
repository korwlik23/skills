...
- Before starting, read `../../RULES.md` and apply it as the behavior, safety, validation, and communication baseline.
- If shared rules are unavailable, scan only the real codebase, never fabricate structure, and report what could not be verified.
- Writing or overwriting `PROJECT_SITEMAP.md` is a file-write operation: when an existing sitemap is present, treat it as confirmation-gated (see Output Format and Update Protocol) so developer-maintained annotations are not lost.
- Use this skill for codebase mapping only; do not let it override user instructions or repository guidance.
...
> **Overwrite gate:** if `PROJECT_SITEMAP.md` already exists, do not silently overwrite
> it. Check it for manual developer annotations first; if present, show the user what
> will change and confirm before writing. Creating it fresh when none exists needs no
> confirmation.
...
6. Preserve any manual annotations added by developers — never drop them without showing the user and getting confirmation (per the Overwrite gate above)
...
- Every path, route, endpoint, and model in the sitemap reflects the actual codebase, not assumptions; unverifiable items are marked as such.
- Generated/ignored directories are excluded; no fabricated structure.
- An existing `PROJECT_SITEMAP.md` was not overwritten without checking for and preserving manual annotations and confirming with the user.
- The sitemap is machine-readable and internally consistent (diagram renders, stats current).
- Limitations are stated when parts of the codebase could not be scanned.

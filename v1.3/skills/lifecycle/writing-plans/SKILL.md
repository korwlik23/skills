...
5b. Apply **Structure Before Code** (`RULES.md` HARD GATE) — every task's file paths
   must map to the design's file/module layout. No task may push a file past the
   ceiling (~500 lines / ~15 public methods); if it would, insert a split task first.
...
| "All these tasks can write to the same file" | That file is becoming a god module. Map tasks to the approved layout. |
...
- Task file paths match the design's file/module layout; no task grows a file past the
  size ceiling without a preceding split task.

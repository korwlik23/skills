...
The "When to Extract" table above is for reviewing existing code. When **creating** a
feature, do not wait for extraction thresholds — start split, per the `RULES.md`
Structure Before Code HARD GATE:

- A feature crossing layers (route → controller → service → model) gets separate files
  per layer from the first commit.
- One controller per resource, max ~7 public actions. An API surface of N resources is
  N controllers — never one `<Product>ApiController` holding every endpoint.
- Never place an entire product under one namespace or `<Product>Module` class; split
  by bounded context (Billing, Media, Publishing, Analytics, …).
- File-size ceiling: ~500 lines / ~15 public methods. A file at the ceiling must be
  split before any new feature lands in it.
- When fixing existing backend code, fix it in place. Do not create a parallel
  service/controller that wraps the broken one; if the file is too large to edit
  safely, stop and propose a split plan.
...
- New code respects Structure at Creation: files per layer, one controller per
  resource, no whole-product namespace, no file over the size ceiling.

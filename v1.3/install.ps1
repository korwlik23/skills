...
.DESCRIPTION
  ...
  intact so each skill's `../../../RULES.md` reference still resolves (per-skill
  `references/` files reach the contract via `../../../../RULES.md`).
...
.EXAMPLE
  # From inside a PowerShell session (Windows PowerShell 5.1 or PowerShell 7+):
  & "C:\Users\kil\Desktop\ai_skills\v1.3\install.ps1" -Project "C:\code\my-app"

  # If blocked by ExecutionPolicy (one-shot bypass, does not change global policy):
  powershell -ExecutionPolicy Bypass -File "C:\Users\kil\Desktop\ai_skills\v1.3\install.ps1" -Project "C:\code\my-app"
#>
[CmdletBinding()]
param(
...
# Verify — skills now live at skills/<bucket>/<name>/SKILL.md, so count SKILL.md
# files instead of top-level directories.
$skillCount  = (Get-ChildItem (Join-Path $dest "skills") -Recurse -Filter "SKILL.md").Count
$bucketCount = (Get-ChildItem (Join-Path $dest "skills") -Directory).Count
$rulesOk     = Test-Path (Join-Path $dest "RULES.md")
$agentsOk    = Test-Path (Join-Path $dest "AGENTS.md")
Write-Host "Installed to: $dest"
Write-Host ("  buckets: {0}  skills: {1}  RULES.md: {2}  AGENTS.md: {3}" -f $bucketCount, $skillCount, $rulesOk, $agentsOk)

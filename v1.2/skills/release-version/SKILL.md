...
description: >
  Use this skill for version management including SemVer bumps, changelog
  generation, Git tagging, release notes, and multi-file version sync.
  Triggers on requests about releasing, bumping version, publishing,
  creating a new version, writing release notes, or updating changelog.
---
...
| Tag created on wrong commit | If the tag was NOT pushed: delete local tag, recreate on the correct commit. If it WAS pushed: prefer a new corrected tag; deleting/moving a published tag is destructive and requires explicit user confirmation per `../../RULES.md` (never force-push a tag silently). |
...
- Do not delete or overwrite existing Git tags.
- Do not publish to registries without explicit user confirmation.
- Do not skip build + test validation before any version bump.
- Do not modify past changelog entries — append corrections instead.
- Do not reuse version numbers that have already been published.
